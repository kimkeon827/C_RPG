...
public class C_Inventory
{
    ...
    {
        GameManager.Inst.Logger.Log($"<#26FF00>{data.itemName}를 1개 얻었습니다. </color>");
    ...
    public bool AddItem(ItemData data, uint index)
    {
        ...
        {
            C_ItemSlot slot = slots[index];   // 해당 인덱스의 슬롯 가져오기
    ...
    public bool RemoveItem(uint slotIndex, uint decreaseCount = 1)
    {
        ...
        {
            C_ItemSlot slot = slots[slotIndex];       // 해당 슬롯에
    ...
    public void MoveItemToTempSlot(uint slotID, uint count)
    {
        ...
        {
            C_ItemSlot fromSlot = slots[slotID];                  // 슬롯 가져오고
